...
export default function TimelineView({ projects }: Props) {
  ...
  // Continuous float index — drives node Y positions via MotionValue pipeline
  // This NEVER triggers a React re-render; it's computed on the animation frame
  const smoothIndex = useTransform(scrollYProgress, (v) => v * projects.length)

  // Discrete index — only used for text styling classes & image display
  // Re-renders are fine here since they only affect CSS classes, not positions
            ...
            >
              ...
              <div className="absolute top-1/2 left-0 -translate-y-1/2">
                {projects.map((project, i) => {
                  ...
                  if (Math.abs(distance) > 12) return null
...
// Each node gets its own useTransform hook, so y/opacity/scale are computed
// directly from the scrollYProgress → smoothIndex MotionValue chain.
// This runs in Framer Motion's animation pipeline — no React re-renders needed
...
interface TimelineNodeProps {
  ...
  itemSpacing: number
  isActive: boolean  // discrete, for CSS class styling only
  containerRef: React.RefObject<HTMLDivElement | null>
}

function TimelineNode({ project, index, smoothIndex, itemSpacing, isActive, containerRef }: TimelineNodeProps) {
  // Frame-perfect Y position: derived directly from scroll via MotionValue
  // (index - smoothIndex) * spacing — computed every animation frame, no setState
  const y = useTransform(smoothIndex, (si) => (index - si) * itemSpacing)

  // Smooth opacity: 1 when this node IS the active one, 0.6 otherwise
  // Uses continuous distance for a smooth crossfade rather than a hard switch
  const opacity = useTransform(smoothIndex, (si) => {
    const dist = Math.abs(index - si)
    // Smoothly interpolate: fully opaque within 0.3 of active, dim beyond 0.7
    return dist < 0.3 ? 1 : dist > 0.7 ? 0.6 : 1 - (dist - 0.3) * (0.4 / 0.4)
  })

  // Smooth dot scale: same continuous approach
  const dotScale = useTransform(smoothIndex, (si) => {
    const dist = Math.abs(index - si)
    return dist < 0.3 ? 1 : dist > 0.7 ? 0.6 : 1 - (dist - 0.3) * (0.4 / 0.4)
  })
  ...
  return (
    ...
    >
      ...
      <Link
        href={`/work/${project.slug.current}`}
        onClick={() => sessionStorage.setItem('workScrollY', containerRef.current?.scrollTop.toString() ?? '0')}
        className={`group flex flex-col justify-center gap-[2px] cursor-pointer transition-all duration-600 hover:opacity-100 ${isActive ? "opacity-100" : "opacity-95"
          }`}
      >
        <span
          className={`uppercase leading-none tracking-tight max-w-[340px] md:max-w-[420px] wrap-break-words transition-all duration-600 ${isActive
            ? "text-black dark:text-white font-bold text-[12px] md:text-[14px]"
            : "text-neutral-300 dark:text-neutral-600 font-medium text-[12px] md:text-[14px]"
            }`}
        >
          {project.title}
        </span>
        ...
        <span
          className={`tracking-[0.15em] transition-all duration-600 ${isActive
            ? "text-black dark:text-white font-medium text-[9px] md:text-[10px]"
            : "text-neutral-400 dark:text-neutral-500 font-normal text-[9px] md:text-[10px]"
            }`}
        >
